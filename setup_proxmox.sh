...
# This script is used to setup Proxmox VE on a Debian-based system.
# Modified from Debian 12 VM setup script from Proxmox VE Community Scripts (https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/vm/debian-vm.sh)
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname" 8 58 dockarr --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="dockarr"
      echo -e "${HOSTNAME}${BOLD}${DGN}Hostname: ${BGN}$HN${CL}"
    else
...
DESCRIPTION=$(
  cat <<EOF
<div align='center'>
  <h2 style='font-size: 24px; margin: 20px 0;'>Dock-arr</h2>
</div>
EOF
)
